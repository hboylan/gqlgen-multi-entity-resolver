...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveManyEntities := func(ctx context.Context, typeName string, reps []map[string]interface{}, idx []int) (err error) {
		...
		case "Todo":
			resolverName, err := entityResolverNameForTodo(ctx, reps[0])
			if err != nil {
			...
			case "findManyTodoByIDs":
				_reps := make([]*model.TodoByIDsInput, len(reps))
				...
				for i, rep := range reps {
					...
					_reps[i] = &model.TodoByIDsInput{
						ID: id0,
					}
				}

				entities, err := ec.resolvers.Entity().FindManyTodoByIDs(ctx, _reps)
				if err != nil {
					return err
				}
